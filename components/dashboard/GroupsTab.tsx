...
export function GroupsTab({ data, months, dataMonths }: Props) {
  ...
  const scaled = useMemo(
    () => (data.groups ?? []).map(g => scaleGroupRow(g, ratio)),
    [data, ratio],
  );

  const slaGroup = useMemo(
    () =>
      (data.sla_group ?? [])
  ...
  const radarData = useMemo(
    () =>
      scaled.slice(0, 6).map(g => ({
        Group:        g.Group.replace(/ Team$/, "").replace(/ /g, "\n"),
        "SLA %":      g["SLA Met %"],
        "Res Rate %": g["Res Rate %"],
        // ── FIX (INFO): Rename to make clear this is normalised (0–100
        //    relative to the highest-volume group, not an absolute count).
        "Vol % (norm)": Math.min(
          100,
          Math.round((g.Total / (scaled[0]?.Total || 1)) * 100),
        ),
      })),
    [scaled],
  );
    ...
    <div className="animate-fade-in space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <Card>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart
              data={[...scaled].sort((a, b) => b.Total - a.Total).slice(0, 8)}
              layout="vertical"
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card>
          ...
          {/* ── FIX (INFO): Clarify that "Vol % (norm)" is volume normalised
               to the highest-volume group (not a raw percentage). */}
          <p className="text-[10px] text-theme-dim mb-2 italic">
            Vol % (norm) is ticket count relative to the busiest group (100 = max volume).
          </p>
          <ResponsiveContainer width="100%" height={240}>
            <RadarChart data={radarData} margin={{ top: 10, right: 20, bottom: 10, left: 20 }}>
              ...
              <Radar name="Vol % (norm)" dataKey="Vol % (norm)" stroke="#f59e0b" fill="#f59e0b" fillOpacity={0.10} />
              <Legend wrapperStyle={{ fontSize: 11 }} />
            </RadarChart>
          </ResponsiveContainer>
        </Card>
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart
              data={[...scaled].sort((a, b) => b["Tickets/Tech"] - a["Tickets/Tech"]).slice(0, 8)}
              layout="vertical"
              margin={{ left: 100, right: 50 }}
            >
              <CartesianGrid strokeDasharray="3 3" stroke="rgb(var(--border))" horizontal={false} />
