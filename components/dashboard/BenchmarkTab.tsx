...
// ── FIX (HIGH): The original getStatus() used string-matching
//    (`metricKey.includes('max') || ... 'reopen' || ... 'mttr'`) to detect
//    lower-is-better metrics, but the escalation key is 'escl_target' — none
//    of those substrings match it, so escalations above target showed green.
//    Replace with an explicit whitelist.
const LOWER_IS_BETTER = new Set(["mttr_target", "reopen_target", "escl_target"]);
...
function getStatus(
  ...
  if (LOWER_IS_BETTER.has(metricKey)) {
    // Lower actual is better (MTTR, reopen rate, escalation rate)
    return actual <= target ? "green" : actual <= target * 1.2 ? "amber" : "red";
  }
  // Higher actual is better (SLA %, FCR %)
  ...
  return (
    <div className="animate-fade-in space-y-4">
      ...
      <Card>
        ...
        {!benchmarks ? (
          <div className="text-xs text-theme-dim py-4">
            No targets configured. Set targets in Admin → Customers → 🎯 Targets.
          </div>
        ) : (
          <table className="w-full text-xs">
            ...
            <tbody>
              {METRICS.map(m => {
                ...
                const actual = m.dataKey
                  ? (dash as Record<string, number>)[m.dataKey] ?? 0
                  : null;
                const status = actual !== null
                  ? getStatus(m.key, target, actual)
                  : "amber";
                return (
                  <tr key={m.key} className="border-b border-theme-border/20">
                    <td className="py-2 pr-4 font-semibold text-theme-text">
                      {m.label}
                    </td>
                    <td className="py-2 px-3 text-right text-theme-dim">
                      {target !== null ? target : <span className="italic">Not set</span>}
                    </td>
                    <td className="py-2 px-3 text-right font-bold text-theme-text">
                      {actual !== null ? actual : "—"}
                    </td>
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {(data.sla_group ?? []).map((g, i) => {
              ...
              const status = getStatus("sla_target", target, met);
              return (
                <tr key={i} className="border-b border-theme-border/20">
                  <td className="py-1.5 font-semibold text-theme-text">
                    {g.assignment_group as string}
                  </td>
                  <td
                    className="py-1.5 px-3 text-right font-bold"
                    style={{ color: met >= 90 ? "#22c55e" : met >= 75 ? "#f59e0b" : "#ef4444" }}
                  >
                    {met}%
                  </td>
                  <td className="py-1.5 px-3 text-right text-theme-dim">
                    {target ?? "—"}
                  </td>
