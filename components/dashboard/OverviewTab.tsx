...
import { getDataMonths, monthRatio } from "@/lib/analytics/dateFilter";
...
export function OverviewTab({ data, months }: OverviewTabProps) {
  ...
  const momBadge = (val: number | null) => {
    ...
    const color = val < 0 ? CHART_COLORS.green : val > 0 ? CHART_COLORS.red : "#6b8fd4";
    return <span style={{ color }} className="font-bold ml-1">{val > 0 ? `▲${val}%` : val < 0 ? `▼${Math.abs(val)}%` : "—"}</span>;
  };
