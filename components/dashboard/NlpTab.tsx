...
const COLORS = [
  "#3d6fd4", "#14b8a6", "#f59e0b", "#a855f7",
  "#ef4444", "#22c55e", "#e879f9", "#f97316",
  "#38bdf8", "#c084fc",
];
    ...
    <div className="animate-fade-in space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-theme-dim">
          {nlp.hasText ? (
            <>
              <b className="text-theme-accent">{nlp.totalAnalysed.toLocaleString()}</b>{" "}
              records analysed · Live NLP
            </>
          ) : (
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart
              data={nlp.top_keywords.slice(0, 12)}
              layout="vertical"
          ...
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie
                ...
                outerRadius={70}
                label={({ name, percent }) =>
                  `${name.split("/")[0]?.trim()} ${Math.round((percent ?? 0) * 100)}%`
                }
                labelLine={false}
              >
                {nlp.rc_counts.slice(0, 8).map((_, i) => (
                  <Cell key={i} fill={COLORS[i % COLORS.length]} />
                ))}
              </Pie>
              <Tooltip />
              {/* ── FIX (LOW): Root cause pie was the only chart in this tab
                   missing a <Legend>, making it impossible to identify slices
                   whose labels are too small to read. */}
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart
              data={nlp.error_counts.filter(e => e.count > 0).sort((a, b) => b.count - a.count).slice(0, 10)}
              layout="vertical"
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart
              data={nlp.res_counts.filter(r => r.count > 0).sort((a, b) => b.count - a.count).slice(0, 10)}
              layout="vertical"
            ...
            <Card>
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                {nlp.phase_dist.map(p => (
                  <div
                    key={p.phase}
                    className="flex items-center gap-2 bg-theme-surface border border-theme-border rounded px-3 py-1.5 text-xs"
                  >
                    <span className="font-semibold text-theme-text">{p.phase}</span>
                    <span className="text-theme-accent font-bold">{p.count}</span>
                  </div>
                ))}
              </div>
            </Card>
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                {nlp.sentiment_dist.map(s => {
                  ...
                  return (
                    <div
                      key={s.sentiment}
                      className="flex items-center gap-2 bg-theme-surface border rounded px-3 py-1.5 text-xs"
                      style={{ borderColor: `${color}44` }}
                    >
                      <span className="font-semibold" style={{ color }}>
                        {s.sentiment}
                      </span>
                      <span className="text-theme-dim">{s.count}</span>
                    </div>
                  );
                })}
              </div>
        ...
        <Card>
          ...
          <div className="grid grid-cols-3 gap-2">
            {nlp.repeat_issues.slice(0, 12).map((r, i) => (
              <div
                key={i}
                className="bg-theme-surface border border-theme-border rounded p-2.5 text-xs"
              >
                <div className="font-bold text-theme-text mb-0.5">{r.caller_id}</div>
                <div className="text-theme-dim">{r.subcategory}</div>
                <div className="text-theme-accent font-bold mt-1">{r.count}× reported</div>
              </div>
            ))}
          </div>
        </Card>
