...
const LINE_COLORS = ["#3d6fd4","#14b8a6","#f59e0b","#ef4444","#a855f7","#22c55e","#e879f9","#f97316","#06b6d4","#84cc16","#fb923c","#38bdf8","#c084fc","#4ade80","#fb7185"];
const PERF_COLOR  = (v: number) => v >= 75 ? "#22c55e" : v >= 55 ? "#f59e0b" : "#ef4444";
type SortKey = "Perf Index" | "Total" | "SLA Met %" | "FCR %" | "Avg Res (hrs)" | "Reopen Rate %";
...
export function TechniciansTab({ data, months, dataMonths }: Props) {
  ...
  const scaled   = useMemo(() => (data.technicians ?? []).map(t => scaleTechnicianRow(t, ratio)), [data, ratio]);
  const sorted   = useMemo(() => [...scaled].sort((a,b) => {
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card>
          <div className="flex justify-between items-baseline mb-2">
            ...
            <span className="text-[10px] text-dark-dim italic">Click bar to view detail</span>
          </div>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart
              data={[...scaled].sort((a,b)=>b.Total-a.Total).slice(0,10)}
              layout="vertical"
        ...
        <Card>
          ...
          <div className="text-[11px] text-dark-dim mb-2">40% SLA · 25% FCR · 20% Reopen · 15% Reassign</div>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart
              data={[...scaled].filter(t=>typeof t["Perf Index"]==="number").sort((a,b)=>(b["Perf Index"]??0)-(a["Perf Index"]??0)).slice(0,10)}
              layout="vertical"
            ...
            >
              ...
              <YAxis type="category" dataKey="Technician" width={95} tick={{ fill:"#6b8fd4", fontSize:10 }} axisLine={false} tickLine={false}/>
              <Tooltip content={<ChartTooltip/>}/>
              <Bar dataKey="Perf Index" radius={[0,4,4,0]}
                fill="#3d6fd4"
      ...
      <Card>
        <div className="flex justify-between items-center mb-2">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[11px] text-dark-dim">Show top</span>
            {[5,8,10,15].map(n => (
              <button key={n} onClick={() => setTopN(n)}
                className={`px-2 py-0.5 rounded text-[11px] border transition-colors ${topN===n ? "border-dark-blue text-dark-accent bg-dark-blue/20 font-bold" : "border-dark-border text-dark-dim"}`}>
                {n}
              </button>
            ))}
          </div>
        ...
        {techTrend.months.length < 2
          ? <div className="py-10 text-center text-dark-dim text-sm">At least 2 months of data needed.</div>
              ...
              <tbody>
                {medAvgData.map((r,i) => (
                  <tr key={i} className="border-b border-dark-border/20">
                    <td className="py-1 pr-2 text-dark-text">{r.name}</td>
                    <td className="py-1 px-2 text-right text-dark-dim">{r.avg}h</td>
                    <td className="py-1 px-2 text-right text-dark-dim">{r.median}h</td>
                    <td className="py-1 pl-2 text-right font-bold"
                      style={{ color: r.gap > 8 ? "#ef4444" : r.gap > 4 ? "#f59e0b" : "#22c55e" }}>
                      +{r.gap}h
                    </td>
                  </tr>
                ))}
      ...
      <Card>
        <div className="flex justify-between items-center mb-3">
          ...
          <div className="flex items-center gap-2 text-[11px]">
            <span className="text-dark-dim">Sort by:</span>
            {(["Perf Index","Total","SLA Met %","FCR %","Avg Res (hrs)","Reopen Rate %"] as SortKey[]).map(k => (
              <button key={k} onClick={() => toggleSort(k)}
                className={`px-2 py-0.5 rounded border transition-colors ${sort===k ? "border-dark-blue text-dark-accent bg-dark-blue/20 font-bold" : "border-dark-border text-dark-dim"}`}>
